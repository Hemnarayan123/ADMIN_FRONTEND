...
const FeatureKits = () => {
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("admin/get-featurekits", param);      
    ...
    const deleteData = async (event) => {
        ...
        if (isConfirmed) {
            ...
            var { data } = await AxiosHelper.deleteData(`admin/delete-featurekits/${_id}`);
    ...
    const dropList = [
        { name: "View", onClick: viewData },
        {
            ...
            onClick: (event) => {
                var data = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
                navigate(`/admin/featureKits/edit/${data._id}`)
            }
        },
                        ...
                        <div className="card-header">
                            <div className="row flex-between-end">
                                <div className="col-auto align-self-center">
                                    <h5 className="mb-0" data-anchor="data-anchor">FeatureKits</h5>
                                </div>
                                    ...
                                    <div className="table-responsive1 ">
                                        <table className="table table-bordered table-striped fs--1 mb-0">
                                            ...
                                            <thead className="bg-200 text-900">
                                                <tr>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "product_name" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="product_name">Product Name</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "product_status" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="product_status">Product Status</th>
                                            ...
                                            <tbody className="list">
                                                {data?.record && data?.record.map((row, i) => {
                                                    return (
                                                        <tr key={i}>
                                                            <td className="fw-bold text-primary cursor-pointer" main-data={JSON.stringify(row)} onClick={viewData}>{row.product_name}</td>
                ...
                <Modal.Body>
                    ...
                    <ul className="list-group">
                        <li className="list-group-item d-flex justify-content-between align-items-center">
                            <label className='fs--1 m-0'>Product Name</label>
                            <span className="fs--1">{initialValues?.product_name}</span>
                        </li>
                        <li className="list-group-item d-flex justify-content-between align-items-center">
                            <label className='fs--1 m-0'>Product Status</label>
                            <span className="fs--1 ps-5 text-center">{initialValues?.product_status}</span>
                        </li>

                        <li className="list-group-item d-flex justify-content-between align-items-center">
                            <label className='fs--1 m-0'>FeatureKits Created Date</label>
