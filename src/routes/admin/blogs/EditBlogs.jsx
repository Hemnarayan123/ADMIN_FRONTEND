...
const EditBlogs = () => {
    ...
    const fetchBlogsData = useCallback(async () => {
        try {
            const { data } = await AxiosHelper.getData(`admin/get-blogs-details/${slug}`);
            if (data?.status === true) {
    ...
    useEffect(() => { fetchBlogsData()}, [slug, fetchBlogsData]);
    ...
    const validationSchema = Yup.object().shape({
        ...
        meta_keywords: Yup.string().required('Meta Keywords is required').min(2, 'Meta Keywords must be at least 2 characters').max(100, 'Meta Keywords must be less than 100 characters'),
                        ...
                        <div className='card-body'>
                            <MyForm
                                ...
                                onSubmit={async (values) => {
                                    const data = await AxiosHelper.putData(`admin/update-blogs/${values.id}`, values, true);
